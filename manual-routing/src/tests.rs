...
#[test]
fn test_forward() { test("/", ContentType::Plain, Status::Ok, "Hello!".to_string()); }
...
fn test_upload() {
  let client = Client::tracked(rocket()).unwrap();
  let expected_body = "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod \
                       tempor incididunt ut labore et dolore magna aliqua"
    .to_string();

  // Upload the body.
  let response = client.post("/upload").header(ContentType::Plain).body(&expected_body).dispatch();
